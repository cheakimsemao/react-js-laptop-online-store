...
const Laptop = (props) => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:3000/brands/${props.match.params.id}/products`).then((res) => {
            setData(res.data);
    ...
    const grid = data.map((item) => (
        <div className={'product-items index'}>
            <Link to={`/Brands/${item.brand}/${item.id}`}>
                <img src={item.imageURL} alt={item.brand} width='300px' height='200px' />
            </Link>
            <p class='product-items-title'>{item.brand}</p>
